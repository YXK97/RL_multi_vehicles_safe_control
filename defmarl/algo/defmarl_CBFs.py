...
from ..utils.utils import jax_vmap, tree_index, tree_where, tree_2nd_index
from ..trainer.data import Rollout
from ..trainer.utils import rollout_p1step
...
class DefMARL_CBFs(Algorithm):
    def __init__(
        ...
        super(DefMARL_CBFs, self).__init__(
            env=env,
            node_dim=node_dim,
            edge_dim=edge_dim,
        ...
        def rollout_fn_single_(cur_params, cur_key, cur_init_graph):
            return rollout_p1step(self._env,
    ...
    def collect(self, params: Params, b_key: PRNGKey) -> Rollout:
        if not self.use_prev_init or self.memory is None:
            rollout_result_p1 = self.rollout_fn(params, b_key)
        else:
        ...
        rollout_result = Rollout(tree_2nd_index(rollout_result_p1.graph, graph_index),
                                 ...
                                 rollout_result_p1.rnn_states[:, :-1],
                                 rollout_result_p1.rewards[:, :-1],
                                 bTah_Hk,
                                 rollout_result_p1.goals[:, :-1],
                                 rollout_result_p1.dones[:, :-1],
                                 rollout_result_p1.log_pis[:, :-1],
                                 tree_2nd_index(rollout_result_p1.next_graph, graph_index),
                                 rollout_result_p1.zs[:, :-1])
