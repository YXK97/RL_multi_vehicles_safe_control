...
def generate_horizontal_path_points(xrange: Array,
    ...
    onen_xs = jnp.linspace(start=xrange[0][None], stop=xrange[0][None] + (num_points + 1) * points_interval, num=num_points,
                         dtype=jnp.float32).T
    onen_ys = start_y * jnp.ones_like(onen_xs)
    onen_thetas_rad = jnp.zeros_like(onen_ys)
    onen_thetas_deg = jnp.zeros_like(onen_ys)
    # state: x y vx vy θ dθdt bw bh
    onen_vs_kmph = jnp.repeat(terminal_vx[None, None], onen_thetas_rad.shape[1], axis=1)
...
class SceneBase(ABC):
    ...
    @abstractmethod
    def make(self) -> Tuple[AgentState, State, ObstState, PathRefs]:
        pass
...
class LaneChangeMiddleStaticEdgeFastMoving(LaneChangeANDOvertakeScene):
    ...
    def make(self) -> Tuple[AgentState, State, ObstState, PathRefs]:
        ...
        sobst_theta = jr.uniform(sobst_theta_key, shape=(), dtype=jnp.float32, minval=-30, maxval=30)
        ...
        return aS_agent_state, aS_goal_init, oS_obst_state, anS_goals
...
class OvertakeEdgeMiddleStaticsEdgeFastMoving(LaneChangeANDOvertakeScene):
    ...
    def make(self) -> Tuple[AgentState, State, ObstState, PathRefs]:
        ...
        sobst1_y = terminal_y
        two_sobst_theta = jr.uniform(sobst_theta_key, shape=(2,), dtype=jnp.float32, minval=-180, maxval=180)
...
def gen_scene_randomly(key: PRNGKey, num_agents: int, num_ref_points: int, xrange: Array, yrange: Array,
                       lane_width: float, lane_centers: Array) -> Tuple[AgentState, State, ObstState, PathRefs]:
    ...
    aS_agent_state, aS_goal_init, oS_obst_state, anS_goals = jax.lax.switch(choose_id, scene_list)
