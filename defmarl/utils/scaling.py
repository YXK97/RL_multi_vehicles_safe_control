...
@jax.jit
def compute_intersections(O: Pos2d, V: Pos2d, A, b, fill: Pos2d, eps=1e-8) -> Array[Pos2d]:
    """计算输入两点的直线与Ax=b各边的交点"""
    # 解析输入直线系数（向量化扩展为(k,)形状，适配A的q条边）
    a0 = V[1] - O[1]
    b0 = O[0] - V[0]
    c0 = V[0] * O[1] - O[0] * V[1]
    max_abs = jnp.max(jnp.stack([jnp.abs(a0), jnp.abs(b0), jnp.abs(c0)], axis=1), axis=1)
    a0 = a0 / max_abs
    b0 = b0 / max_abs
    c0 = c0 / max_abs
...
@jax.jit
def filter_ray_direction(k2_intersections: Array[Pos2d], O: Pos2d, V: Pos2d, fill: Pos2d) -> Array[Pos2d]:
    """筛选与射线方向一致”的交点（射线：从O指向V）"""
    # 射线方向向量
    dir_vec = V - O  # (2,)
    # 交点相对于r1的向量：inter_vec = 交点 - r1
    inter_vec = k2_intersections - V  # (k, 2)
...
@jax.jit
def filter_in_bound(k2_intersections: Array[Pos2d], A, b, fill: Pos2d) -> Array[Pos2d]:
    """筛选满足Ax<=b的点"""
    def _single_in_bound(intersection: Pos2d, A, b) -> bool:
        return (jnp.dot(A, intersection) <= b).all()
...
def scaling_calc(s1: State, s2: State) -> Array:
    ...
    nl2_intersections = jax.vmap(compute_intersections, in_axes=(None, 0, None, None, None))(
        O, n_P, A1, b1, fill=O+1e-8)
    nl_dist = jnp.linalg.norm(nl2_intersections - O, axis=-1)
    nl_dist0 = jnp.linalg.norm(n_P - O, axis=-1)[:, None].repeat(nl_dist.shape[1], axis=1)
    nl_scaling = nl_dist / nl_dist0
    scaling = jnp.array([mk_scaling.min(), nl_scaling.min()]).min()

    # 判断S1缩放中心是否在S2中
    O_in_S2 = jnp.where((A2@O-b2<=0).all(), -1., 1.)
