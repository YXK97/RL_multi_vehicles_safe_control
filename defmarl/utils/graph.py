...
class GraphsTuple(tuple, Generic[_State, _EnvState]):
    ...
    def __new__(
        ...
        tup = (n_node, n_edge, nodes, edges, states, receivers, senders, node_type, env_states)
...
class GetGraph(NamedTuple):
    ...
    def to_padded(self) -> GraphsTuple:
        ...
        """首先，不管原始agent、目标点、障碍物有多少个，只引入1个dummy node（辅助点），用于将edge mask为false，即通讯范围之外或自环的边转换成辅助点的自环；
        其次，原始的agent不存在自环，所有自环都通过edge mask转换成了辅助点的自环；
        最后，辅助点的自环不影响其他点的网络计算，因为辅助点的type为-1，整个图里就它一个type为-1的点。"""
        ...
        n_nodes, n_edges = self.n_nodes + 1, e_edge_feats.shape[0] # n_nodes+1就是因为增加了一个辅助点
